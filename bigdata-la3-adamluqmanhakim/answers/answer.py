...
def data_frame(filename, n):
    ...
    plants = sc.textFile(filename) \
        .map(lambda l: l.split(',')) \
        .zipWithIndex().map(lambda p: Row(id=p[1], plant=p[0][0], items=p[0][1:]))
    df = spark.createDataFrame(plants)
    return toCSVLine(df.limit(n))
...
def frequent_itemsets(filename, n, s, c):
    ...
    return toCSVLine(model.freqItemsets \
                     .withColumn("item_cnt", size(model.freqItemsets.items)) \
                     .sort(desc("item_cnt"), desc("freq")) \
                     .select('items', 'freq') \
                     .limit(n))
...
def association_rules(filename, n, s, c):
    ...
    return toCSVLine(model.associationRules \
                     .withColumn("a_cnt", size(model.associationRules.antecedent)) \
                     .sort(desc("a_cnt"), desc("confidence")) \
